...
def preprocess_data(raw_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series, List[str], MinMaxScaler, OneHotEncoder]:
    """
    ...
    Returns:
        Tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series, List[str], MinMaxScaler, OneHotEncoder]:
            Processed training inputs, training targets, validation inputs, validation targets, input columns, scaler, and encoder.
    """
    ...
    return train_inputs, train_targets, val_inputs, val_targets, input_cols.tolist(), scaler, encoder
...
def preprocess_new_data(new_data: pd.DataFrame, scaler: MinMaxScaler, encoder: OneHotEncoder, numeric_cols: list, categorical_cols: list) -> pd.DataFrame:
    ...
    new_data = new_data.drop(columns=categorical_cols)  # Remove original categorical columns
    return new_data